(() => {
  ...
  function applyServer(items){
    ...
    items.forEach(item=>{
      ...
      if(!task)return;
      const cycleKey=String(item.cycleKey||'').trim();
    ...
    for(const {item,task} of latest.values()){
      const member=String(item.memberKey||'');
      if(!member)continue;
  ...
  async function fetchShared({silent=false}={}){
    ...
    try{
      const k=keys(),u=new URL(API);
      u.searchParams.set('action','read');
      u.searchParams.set('dailyKey',k.daily);
      u.searchParams.set('weeklyKey',k.weekly);
      ...
      console.warn('[Task Journal v6] read failed',e);
  ...
  async function writeShared(task,checked){
    ...
    try{
      ...
      setSync('已儲存到共用進度');
      setTimeout(()=>fetchShared({silent:true}),700);
      return true;
    }catch(e){
      console.warn('[Task Journal v6] write failed',e);
  ...
  function refreshOpenDialog(){
    ...
    [['daily',dp],['weekly',wp]].forEach(([k,p])=>{const g=d.querySelector(`[data-task-group="${k}"]`);if(!g)return;const c=g.querySelector('[data-group-count]'),b=g.querySelector('[data-group-progress]');if(c)c.textContent=`${p.done} / ${p.total}`;if(b)b.style.width=`${p.pct}%`;});
  ...
  document.addEventListener('click',e=>{
    ...
    selectedRole=Number(row.dataset.taskRoleIndex)||0;selectedMember=currentMemberKey();
    ...
    const id=vals.at(-1)||'';selectedRoleName=id&&id!=='—'?id:tag;selectedLabel=id&&id!=='—'?`${tag}・${id}`:tag;
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(!row||!['Enter',' '].includes(e.key))return;e.preventDefault();row.click();
  });
  ...
  document.addEventListener('change',e=>{
    ...
    const s=load(),r=roleState(s,selectedMember,selectedRole);r[task.cycle][task.id]=input.checked;save(s);
  ...
  setInterval(()=>{if(document.visibilityState==='visible'&&($('#memberDialog')?.open||$('#mabiTaskDialog')?.open))fetchShared({silent:true});},4000);
