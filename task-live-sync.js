(() => {
  ...
  function roleState(s,m,i){s.members[m]||={roles:{}};s.members[m].roles||={};s.members[m].roles[i]||={daily:{},weekly:{}};return s.members[m].roles[i];}
  function parseTime(v){const p=Date.parse(String(v||'').replace('上午','AM').replace('下午','PM'));return Number.isFinite(p)?p:0;}
  function memberKey(){return [$('#modalGuild')?.textContent?.trim(),$('#modalMainId')?.textContent?.trim(),$('#modalLine')?.textContent?.trim()].join('||');}
  function progress(r,list){const done=list.filter(t=>!!r?.[t.cycle]?.[t.id]).length;return {done,total:list.length,pct:Math.round(done/list.length*100)};}

  function applyLatest(items){
  ...
  function refreshOpenDialog(){
    ...
    let roleIndex=rows.findIndex(row=>title.includes(row.querySelector('.role-tag')?.textContent?.trim()||''));
    if(roleIndex<0)roleIndex=0;
    const s=load(),r=roleState(s,memberKey(),roleIndex),dp=progress(r,DAILY),wp=progress(r,WEEKLY);
    TASKS.forEach(t=>{
      const input=dialog.querySelector(`[data-task-id="${t.id}"]`); if(!input)return;
      const checked=!!r?.[t.cycle]?.[t.id];
      input.checked=checked;
      input.closest('.mabi-task-item')?.classList.toggle('is-done',checked);
    });
    const map=[['[data-total-count]',`${dp.done+wp.done} / ${dp.total+wp.total}`],['[data-summary-daily]',`${dp.done} / ${dp.total}`],['[data-summary-daily-pct]',`${dp.pct}% 完成`],['[data-summary-weekly]',`${wp.done} / ${wp.total}`],['[data-summary-weekly-pct]',`${wp.pct}% 完成`]];
    map.forEach(([sel,text])=>{const n=dialog.querySelector(sel);if(n)n.textContent=text;});
  ...
  async function sync(){
    ...
    try{
      ...
      applyLatest(data.items);
      refreshRoleCards();
      refreshOpenDialog();
    }catch(error){console.warn('[Task Live Sync] sync failed:',error);}
    finally{busy=false;}
  }
  ...
  document.addEventListener('click',e=>{if(e.target.closest('#memberGrid .member-card,#modalRoles .role-row'))setTimeout(sync,150);});
  setInterval(()=>{if($('#memberDialog')?.open||$('#mabiTaskDialog')?.open)sync();},15000);
})();
